...
export async function adminRoutes(server: FastifyInstance, authService?: AuthService) {
  ...
  server.get('/admin/dashboard', { preHandler: requireAdmin }, async (request, reply) => {
    // Get basic stats
    const totalUsers = await insforgeClient.find('users', {}).then(users => users.length);
    const totalOrgs = await insforgeClient.find('organizations', {}).then(orgs => orgs.length);
    const activeApiKeys = await insforgeClient.find('api_keys_vault', { active: true }).then(keys => keys.length);
  ...
  server.get('/admin/users', { preHandler: requireAdmin }, async (request, reply) => {
    ...
    const total = await insforgeClient.find('users', filter).then(users => users.length);
  ...
  server.get('/admin/organizations', { preHandler: requireAdmin }, async (request, reply) => {
    ...
    const total = await insforgeClient.find('organizations', {}).then(orgs => orgs.length);
  ...
  server.get('/admin/modules', { preHandler: requireAdmin }, async (request, reply) => {
    ...
    const moduleConfig = modules.reduce((acc, mod) => {
  ...
  server.get('/admin/billing/config', { preHandler: requireAdmin }, async (request, reply) => {
    ...
    const billingConfig = config.reduce((acc, setting) => {
  ...
  server.get('/admin/white-label', { preHandler: requireAdmin }, async (request, reply) => {
    ...
    const whiteLabelConfig = settings.reduce((acc, setting) => {
  ...
  server.get('/admin/audit-log', { preHandler: requireAdmin }, async (request, reply) => {
    ...
    const total = await insforgeClient.find('audit_log', {}).then(logs => logs.length);
